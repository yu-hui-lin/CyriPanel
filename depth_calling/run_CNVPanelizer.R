...
# --- NEW IN CYRIPANEL: CNVPanelizer R wrapper script ---
# This script implements reference-panel based CNV detection for targeted sequencing panels.
...
# --- Parse command-line arguments: sample BAM, BED file, reference BAM directory, output directory. ---
args <- commandArgs(trailingOnly=TRUE)
if (length(args) != 4) {
  stop("Usage: Rscript run_CNVPanelizer.R <sample_bam> <bed_file> <reference_dir> <output_dir>")
}
...
tryCatch({
  ...
  if (!file.exists(sampleFilenames)) {
    stop(paste("Test sample BAM file not found at:", sampleFilenames))
  } else {
    message(paste("Test sample BAM file:", sampleFilenames))
  }
