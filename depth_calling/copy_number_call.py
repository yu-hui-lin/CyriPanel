...
# --- MODIFIED: Increase ERROR_RATE from 0.01 to 0.1 following BCyrius update.
# Higher error rate accounts for increased noise in targeted sequencing data compared to WGS,
# making Poisson model more tolerant of sequencing artifacts and alignment errors in panels. ---
ERROR_RATE = 0.1
# --- END OF MODIFICATION ---

# --- MODIFIED: Add thresholds for targeted panel-specific CN calling challenges.
# MIN_ALLELE_FRACTION_FOR_CN1_TARGETED prevents false positives from low-level noise when calling CN=1.
# AMBIGUOUS_CALL_THRESHOLD resolves CN=2-vs-3 ambiguity by preferring CN=2 when confidence <0.85. ---
...
def call_reg1_cn(full_cn, count_reg1, count_reg2, min_read=0):
    ...
    if full_cn == 1:
        if total_reads_at_site > 20:  # Only apply this logic with sufficient read depth
            ...
            if allele_fraction >= 0.50 and count_reg1 > min_read:
...
def process_raw_call_gc(cn_prob, post_cutoff, keep_none=True):
    ...
    for cn_call in cn_prob:
        ...
        # --- MODIFIED: Resolve ambiguous CN=2-vs-3 calls by preferring CN=2 when confidence is marginal.
        # Targeted panels often produce ambiguous signals where model returns [3, 0.75, 2, 0.25] -> top call is CN=3 with 75% confidence but CN=2 is close behind.
        # If confidence <0.85, prefer conservative CN=2 call to avoid false positive duplications, which are more problematic than missing true duplications. ---
